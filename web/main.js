...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    let atmotube, atmotubeDropDown, binSizes, checkZoomLevel, colors, currentBinSize, currentLayer, dataAggregationText, featureDropDown, features, get_data, indexFile, layers, layersControl, layout, map, noLocationDataText, numAtmotubes, period, periodDropDown, plot_config, ranges, resetHeatMap, setBinSize, school;
    ...
    async function init() {
        ...
        get_data = async function(data_url, type) {
            ...
            let fetch_response = await fetch(data_url, {
                'cache': 'force-cache'
            });
        ...
        const indexFiles = {
            'barkerend': 'https://www.googleapis.com/drive/v2/files/15JI3NocKDg35BfiICLiG2IcJMf10a-Nw?alt=media&key=AIzaSyA6LgT8w7V7syKB6Deq0nxjpxX1HIalo6k',
            'beckfoot_allerton': 'https://www.googleapis.com/drive/v2/files/1C5w5UxoSCePD4AEfUsfCN_3nlurBO1IM?alt=media&key=AIzaSyA6LgT8w7V7syKB6Deq0nxjpxX1HIalo6k',
            'beckfoot_heaton': 'https://www.googleapis.com/drive/v2/files/16pJZKjQmgcibCfL3DgaRJc5BXew9zVAB?alt=media&key=AIzaSyA6LgT8w7V7syKB6Deq0nxjpxX1HIalo6k',
            'brackenhill': 'https://www.googleapis.com/drive/v2/files/1xMKjj86CKYcZ10bcsXXW1voQjURQacXo?alt=media&key=AIzaSyA6LgT8w7V7syKB6Deq0nxjpxX1HIalo6k',
            'dixons': 'https://www.googleapis.com/drive/v2/files/1ywS_k71YIEmzJp6aq3UNOmryWqi4dM7o?alt=media&key=AIzaSyA6LgT8w7V7syKB6Deq0nxjpxX1HIalo6k',
            'home_farm': 'https://www.googleapis.com/drive/v2/files/1Qb4CY0QPR7PZSl_yPfg9ZemDopyKjY50?alt=media&key=AIzaSyA6LgT8w7V7syKB6Deq0nxjpxX1HIalo6k',
            'st_barnabas': 'https://www.googleapis.com/drive/v2/files/1CjMQrY6HptcGTZXLSVIxUw105AmPq9dO?alt=media&key=AIzaSyA6LgT8w7V7syKB6Deq0nxjpxX1HIalo6k',
            'st_johns': 'https://www.googleapis.com/drive/v2/files/1Ne0-iDHCEi4HLPoLi8P8c6rvb1SLZYYR?alt=media&key=AIzaSyA6LgT8w7V7syKB6Deq0nxjpxX1HIalo6k',
            'st_stephens': 'https://www.googleapis.com/drive/v2/files/1tUb4cLAFp9TsdXLqwTZSMJLbiO7vDo4j?alt=media&key=AIzaSyA6LgT8w7V7syKB6Deq0nxjpxX1HIalo6k',
            'whetley': 'https://www.googleapis.com/drive/v2/files/1MIiipSh9qCLXqgCGAcNlXUFZQSZSaClT?alt=media&key=AIzaSyA6LgT8w7V7syKB6Deq0nxjpxX1HIalo6k'
        };
        // indexFile = await get_data(indexFiles[school], 'index');
        indexFile = await get_data('random_data_shrinked_encrypted_indexes/' + school + '/' + school + '_index.json', 'index');
    ...
    async function plotTimeSeries() {
        ...
        for (let i = 1; i < numAtmotubes + 1; i++) {
            if (period == 'alltime') {
                // newTimeSeriesData = await get_data('https://www.googleapis.com/drive/v2/files/' + indexFile['data']['alltime']['timeseries'][i] + '?alt=media&key=AIzaSyA6LgT8w7V7syKB6Deq0nxjpxX1HIalo6k', 'timeseries');
                newTimeSeriesData = await get_data(indexFile['data']['alltime']['timeseries'][i], 'timeseries');
            } else {
                // newTimeSeriesData = await get_data('https://www.googleapis.com/drive/v2/files/' + indexFile['data'][period.split(',')[0]][period.split(',')[1]]['timeseries'][i] + '?alt=media&key=AIzaSyA6LgT8w7V7syKB6Deq0nxjpxX1HIalo6k');
    ...
    async function plotHeatMap() {
        ...
        if (period.split(',')[0] == 'daily' && atmotube != 'all') {
            ...
            // heatMapData = await get_data('https://www.googleapis.com/drive/v2/files/' + indexFile['data']['daily'][period.split(',')[1]]['heatmap'][atmotube] + '?alt=media&key=AIzaSyA6LgT8w7V7syKB6Deq0nxjpxX1HIalo6k', 'heatmap');
            ...
            for (const binSize of binSizes) {
                ...
                if (period == 'alltime') {
                    // heatMapData[binSize] = await get_data('https://www.googleapis.com/drive/v2/files/' + indexFile['data']['alltime']['heatmap'][atmotube][binSize] + '?alt=media&key=AIzaSyA6LgT8w7V7syKB6Deq0nxjpxX1HIalo6k', 'heatmap');
                    heatMapData[binSize] = await get_data(indexFile['data']['alltime']['heatmap'][atmotube][binSize], 'heatmap');
                } else {
                    // heatMapData[binSize] = await get_data('https://www.googleapis.com/drive/v2/files/' + indexFile['data'][period.split(',')[0]][period.split(',')[1]]['heatmap'][atmotube][binSize] + '?alt=media&key=AIzaSyA6LgT8w7V7syKB6Deq0nxjpxX1HIalo6k', 'heatmap');
